...
const FairValueGapDetector_1 = require("../detectors/FairValueGapDetector");
...
describe('FairValueGapDetector', () => {
    ...
    beforeEach(() => {
        db = [];
        mockRepository = {
            save: jest.fn().mockImplementation(async (fvg) => {
                db.push(JSON.parse(JSON.stringify(fvg)));
            }),
            saveAll: jest.fn().mockImplementation(async (fvgs) => {
            ...
            update: jest.fn().mockImplementation(async (fvg) => {
        ...
        EventBus_1.EventBus.getInstance().removeAllListeners();
    });
    test('Bullish FVG detection', async () => {
        const detector = new FairValueGapDetector_1.FairValueGapDetector(mockRepository, Fvg_1.MitigationType.Touch);
        const history = [
        ...
        expect(db[0].type).toBe(Fvg_1.FvgType.Bullish);
    ...
    test('Bearish FVG detection', async () => {
        ...
        await detector.initialize('binance', 'BTC/USDT', '1h', history);
        expect(db.length).toBe(1);
        expect(db[0].type).toBe(Fvg_1.FvgType.Bearish);
    ...
    test('Touch Mitigation logic (Bullish and Bearish)', async () => {
        ...
        EventBus_1.EventBus.getInstance().on('market.fvg.mitigated', () => {
    ...
    test('Partial Mitigation logic', async () => {
        ...
        const detector = new FairValueGapDetector_1.FairValueGapDetector(mockRepository, Fvg_1.MitigationType.Partial);
        const history = [
            { timestamp: 1000, open: 10, high: 12, low: 9, close: 11, volume: 100 },
            { timestamp: 2000, open: 11, high: 14, low: 11, close: 13, volume: 100 },
    ...
    test('Full Mitigation logic', async () => {
        ...
        const detector = new FairValueGapDetector_1.FairValueGapDetector(mockRepository, Fvg_1.MitigationType.Full);
        const history = [
            { timestamp: 1000, open: 10, high: 12, low: 9, close: 11, volume: 100 },
            { timestamp: 2000, open: 11, high: 14, low: 11, close: 13, volume: 100 },
            { timestamp: 3000, open: 13, high: 18, low: 16, close: 17, volume: 100 },

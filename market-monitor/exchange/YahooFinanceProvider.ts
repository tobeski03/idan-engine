...
export class YahooFinanceProvider implements ExchangeProvider {
  private supportedExchanges = new Set(['yahoo', 'yahoofinance', 'forex']);
  ...
  public async fetchCandles(
    ...
    const targetTimeframeMs = timeframeToMs(timeframe);
    ...
    return this.retry(async () => {
      Logger.debug(`Fetching candles from Yahoo Finance: ${url}`);
   ...
   */
  private aggregateCandles(candles: Candle[], targetTimeframeMs: number): Candle[] {
  ...
  private async retry<T>(
    ...
    try {
      ...
      if (retries <= 0) {
        Logger.error(`Yahoo Finance request failed after max retries`, error);
        throw error;
      }
      
      Logger.warn(`Yahoo Finance request failed: ${error.message || error}. Retrying in ${delay}ms... (Retries left: ${retries})`);

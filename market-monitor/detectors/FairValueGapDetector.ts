...
import { Fvg, FvgType, MitigationType, checkMitigation } from '../models/Fvg';
import { Candle } from '../models/Candle';
import { EventBus } from '../events/EventBus';
import { Logger } from '../utils/Logger';

export class FairValueGapDetector implements MarketDetector {
  public readonly id = 'FairValueGapDetector';
  private recentCandles: Map<string, Candle[]> = new Map();

  constructor(
    private readonly repository: FvgRepository,
    private readonly mitigationType: MitigationType
  ) {}
  ...
  public async initialize(
    ...
    Logger.info(`Initializing FVG Detector for ${key} with ${history.length} historical candles.`);

    if (history.length < 3) {
      Logger.warn(`Insufficient candle history (${history.length}) for initialization.`);
      this.recentCandles.set(key, [...history]);
      return;
    }

    const detectedFvgs: Fvg[] = [];
    ...
    for (let i = 2; i < history.length; i++) {
      ...
      if (fvg) {
        ...
        for (let j = i + 1; j < history.length; j++) {
          if (checkMitigation(fvg, history[j])) {
            mitigated = true;
            break;
          }
        }
    ...
    if (detectedFvgs.length > 0) {
      ...
      Logger.info(`Found and saved ${detectedFvgs.length} active FVGs for ${key}.`);
    } else {
      Logger.info(`No active FVGs found during initialization for ${key}.`);
    }
  ...
  public async processNewCandle(
    ...
    for (const candle of newCandles) {
      ...
      const stillActive: Fvg[] = [];
      for (const fvg of activeFvgs) {
        if (checkMitigation(fvg, candle)) {
          fvg.state = 'mitigated';
          await this.repository.update(fvg);
          EventBus.getInstance().emit('market.fvg.mitigated', { fvg, candle });
          Logger.info(`FVG mitigated: ${fvg.id} at candle timestamp ${candle.timestamp}`);
        } else {
      ...
      if (cached.length === 3) {
        ...
        if (newFvg) {
          ...
          if (!existing) {
            ...
            EventBus.getInstance().emit('market.fvg.created', { fvg: newFvg });
            Logger.info(`New FVG detected and saved: ${newFvg.id}`);
          }
        }
      }
    }
